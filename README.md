...
掲示板や Discord での議論から「これはカード頁の FAQ に載せておきたい」という Q&A があれば、[コミュニティFAQの収録案フォーム](../../issues/new?template=faq-proposal.yml) で提案してください。管理人以外の方の提案も歓迎です。
...
3. 異論が無ければ管理人がサイトのカード頁へ「コミュニティ」FAQとして収録し、Issue を閉じます（Issue へのリンクがカード頁に残ります）。締切ではないので、それ以降のご意見も歓迎です。収録後でも直します
4. 根拠が割れて公式が沈黙している題材は収録せず、掲示板での議論に留めます（理由を書いて閉じます）

起票すると、目処の日付を入れたレビュー案内が自動でコメントされます。英語か日本語の質問・回答が揃っていないときは `needs-en` / `needs-ja` のラベルも自動で付きます（片言語のままの起票でも構いません。訳を書いてくださる方も歓迎で、無ければ管理人が訳します）。
...
コミュニティFAQの収録案は、管理人がサイト側で登録した時点で反映されます（数分）。
